	...
	public void TimerMainNowMinute1(){
		...
		if(list.size()>0){
			for(SourceVisitBean svb:list){
				String partName = "";
				partName = pd.getChannelNameByChannelId(svb.getChannelId(), svb.getSiteCode());
				String channelId = "";
				...
				if(!"null".equals(partName) && !"null".equals(originName) && !"null".equals(channelId) && !"null".equals(channelName)){
					VisitLogBean vlb = new VisitLogBean();
					vlb.setDate(timeArr[0]);
					vlb.setHour(timeArr[1]);
					vlb.setMinute(timeArr[2]);
					vlb.setChannelId(channelId);
					vlb.setChannelName(channelName);
					vlb.setPartId(svb.getChannelId());
					vlb.setPartName(partName);
					vlb.setOriginId(svb.getOriginId());
					vlb.setOriginName(originName);
					vlb.setSiteCode(svb.getSiteCode());
					vlb.setStartPv(lxmvd.findStartPvCount(svb.getOriginId(), svb.getChannelId(), svb.getSiteCode(), timeArr[3], timeArr[4]));
					vlb.setStartUv(lxmvd.findStartUvCount(svb.getOriginId(), svb.getChannelId(), svb.getSiteCode(), timeArr[3], timeArr[4]));
